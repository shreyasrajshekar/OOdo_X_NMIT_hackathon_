...
plus schema, RLS, functions and storage applied and living in `supabase/`. Not wired up
...
14 tables plus a derived `leave_balances` view, 4 enums, 33 RLS policies, 9 triggers and 4
storage buckets. The rules that matter live in Postgres rather than in the app, so they hold
