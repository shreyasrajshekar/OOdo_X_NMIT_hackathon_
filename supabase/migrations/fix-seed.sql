-- Demo rows for the two Test profiles.
--
-- Guarded, for the same reason 003_seed_data.sql is: these ids only exist if
-- the matching auth users were created first, and leave_balance.employee_id is
-- a foreign key onto profiles(id). On a clean database there is nothing to
-- attach to, so this raised a foreign key violation and took `supabase db push`
-- down with it. It works on our project only because those two profiles happen
-- to be there.
--
-- Unnumbered, so it sorts after 009 and runs last. Left that alone rather than
-- renaming it, since renaming registers a new version and re-runs it.
--
-- For a full demo dataset use `npm run seed`.
...
BEGIN
    ...
    IF v_present < 2 THEN
        RAISE NOTICE 'skipping fix-seed: Test profiles not present (found %/2). Run npm run seed instead.', v_present;
        RETURN;
    END IF;
